...
本系统的权限是很重要的一部分，在用户登录时就会返回当前用户的权限列表，然后动态生成路由和菜单，每次请求时都会对接口权限就行判断。
